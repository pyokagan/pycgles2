...
def gen_ctxinit(registry, dest):
    ...
    out('if (!dot) {')
    out('    PyErr_SetString(PyExc_RuntimeError, "Could not parse OpenGL version");')
    out('    return -1;')
    out('}')
    ...
    out('    PyErr_SetString(PyExc_ValueError, "Invalid OpenGL version");')
    out('    return -1;')
    out('}')
    out('if (strncmp(s, "OpenGL ES", sizeof(char)*9) != 0) {')
    out('    PyErr_SetString(PyExc_ValueError, "Context is not OpenGL ES");')
    out('    return -1;')
    out('}')
